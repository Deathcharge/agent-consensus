...
- Review priority: Run CI and clean-wheel/examples.
- Review priority: confirm legacy callers, package ownership, and license.
- Review priority: then release 0.2.0.
...
- Static audit cannot confirm the declared 95% coverage gate, cross-version CI, wheel contents, or examples.
- No published distribution, release tag, provenance workflow, or confirmed package-index ownership.
- Compatibility aliases do not recreate the old provider-specific API; downstream users, if any, need an explicit migration check.
- README calls the branch a release candidate before independent CI/artifact evidence is recorded in the audit.
